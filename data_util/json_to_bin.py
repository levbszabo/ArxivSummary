...
def _to_string(lst):
  ...
  if isinstance(lst[0], six.string_types):
    ...
    print(type(lst[0]))
    raise AttributeError(type(lst[0]),'Unacceptable format of list to return to string')
  # ret = ' '.join(lst)
  return ret
...
def write_to_bin(infile, outfile, vocab_file=False):
  ...
  print(num_articles)
  num_articles_pad = len(str(num_articles//1000))
  chunk = 0
  chunk_path_data = outfile.split('.')[0]
  chunk_path = f"chunked/{chunk_path_data}_{chunk:0>{num_articles_pad}}.bin"
  print(chunk_path)
  chunk_writer = open(chunk_path,'wb')
  idx = 0
  ...
  #   tf_example.features.feature['article_id'].bytes_list.value.extend([article_id])
    ...
    abstract_str = _to_string(data['abstract_text'])
    # print(abstract_str)
    tf_example.features.feature['abstract'].bytes_list.value.extend([abstract_str.encode('utf-8', 'ignore')])

    sections_str = _to_string(data['sections'])
    # print(sections_str)
    tf_example.features.feature['article'].bytes_list.value.extend([sections_str.encode('utf-8', 'ignore')])
  ...
  #   tf_example.features.feature['labels'].bytes_list.value.extend([labels.encode('utf-8', 'ignore')])
    ...
    chunk_writer.write(struct.pack('q', str_len))
    # s format is bytes (char[])
    chunk_writer.write(struct.pack('%ds' % str_len, tf_example_str))
    if idx % 5 == 0:
